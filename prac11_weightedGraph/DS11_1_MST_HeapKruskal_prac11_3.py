...
크루스칼의 MST코드에서는 가중치가 가장 작은 간선을 뽑기 위해 간선을 가중치의 오름차순으로 정렬했다.
힙을 사용하는 방법으로 코드를 수정하라. 힙을 사용한다면 가중치가 작은 간선을 뽑아야 하기 때문에 최소 힙을 사용해야 한다.
파이썬의 heapq 모듈을 사용해도 좋다.
...
print('MST By Heap Kruskal Algorithm')
